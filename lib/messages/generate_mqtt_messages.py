"""Generate MQTT message classes from a TOML file."""
...
@click.argument('toml_path',
                ...
                required=False,
                default=str(paths.REPO_PATH / 'messages' / 'bot_quickstart_msgs.toml'))
def main(toml_path):
    """
    Generate MQTT message classes from a TOML file.
    ...
    output_dir = paths.REPO_PATH / 'lib' / 'messages'
    ...
    for message_name, fields in messages.items():
        ...
        with open(class_file, 'w') as f:
            ...
            f.write("import json\n\n")
            f.write("from lib.messages.mqtt_message_base import MqttMessageBase\n\n\n")
            f.write(f"class {class_name}(MqttMessageBase):\n")
            f.write(f"    \"\"\"MQTT message class for {message_name}.\"\"\"\n\n")
            ...
            f.write("\n    def __init__(self, ")
            f.write(", ".join([
                f"{field}: {type_mapping.get(value[0], 'str') if value[1] == '1' else ('np.ndarray[' + type_mapping.get(value[0], 'str') + ']' if value[0] in ['f32', 'd64', 'u8', 'u64'] else 'list[' + type_mapping.get(value[0], 'str') + ']')} | None = None"
                for field, value in fields.items() if not field.startswith('topic_')
            ]))
            f.write("):\n")
            f.write("        \"\"\"Initialize the message class with given fields.\"\"\"\n")
            for field, value in fields.items():
                if not field.startswith('topic_') and isinstance(value, list) and len(value) == 2:
            ...
            f.write("            raise Exception(f'Error converting to payload: {e}') from e\n")
            ...
            f.write("            raise Exception(f'Error converting from payload: {e}') from e\n")

    # Update topic_to_message_type.py
    topic_to_message_map = {}
    for message_name, fields in messages.items():
        class_name = message_name.upper() + '_MSG'
        module_name = f"lib.messages.{class_name.lower()}"
    ...
    with open(paths.REPO_PATH / 'lib' / 'messages' / 'topic_to_message_type.py', 'w') as f:
        ...
        # Add sorted import statements for each message class
        sorted_message_classes = sorted(set(topic_to_message_map.values()), 
                                     key=lambda x: x.__name__.lower())
        for message_class in sorted_message_classes:
            f.write(f"from lib.messages.{message_class.__name__.lower()} import {message_class.__name__}\n")
            imported_classes.add(message_class)

        # Define topic variables
        f.write("\n# Topic variables\n")
        for _message_name, fields in messages.items():
            for field, value in fields.items():
                if field.startswith('topic_'):
