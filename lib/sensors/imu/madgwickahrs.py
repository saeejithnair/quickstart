...
class MadgwickAHRS:
    ...
    def update(self, gyroscope, accelerometer, magnetometer):
        ...
        if norm(accelerometer) == 0:
            warnings.warn("accelerometer is zero", stacklevel=2)
            return
        accelerometer /= norm(accelerometer)

        # Normalise magnetometer measurement
        if norm(magnetometer) == 0:
            warnings.warn("magnetometer is zero", stacklevel=2)
            return
        ...
        gyroscopeQuat = gyroscopeQuat + (q.conj() * stepQuat) * 2 * self.sample_period * self.zeta * -1
