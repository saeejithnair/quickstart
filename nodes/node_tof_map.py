...
from lib.messages.topic_to_message_type import TOPIC_TOF_MAP

# -----------------------------------------------------------------------------
# MQTT Setup
# -----------------------------------------------------------------------------
mqtt_publisher = MQTTPublisher(broker_address="localhost", topic_to_message_map={TOPIC_TOF_MAP: TOF_MAP_MSG})
mqtt_publisher.run()
...
try:
    while True:
        ...
        if all_sensor_data:
            ...
            # Add grid to payload
            msg = TOF_MAP_MSG(timestamp=time.time(),
                               sensors=combined_sensor_data,
                               occupancy_grid_data=grid_list,
                               occupancy_grid_height=len(grid_list),
                               occupancy_grid_width=len(grid_list[0]),
                               occupancy_grid_resolution=GRID_RESOLUTION,
                               occupancy_grid_min_x=GRID_MIN_X,
                               occupancy_grid_max_x=GRID_MAX_X,
                               occupancy_grid_min_y=GRID_MIN_Y,
                               occupancy_grid_max_y=GRID_MAX_Y)
            mqtt_publisher.publish_msg(TOPIC_TOF_MAP, msg)
